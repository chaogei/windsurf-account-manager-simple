  ...
  >
    <div class="tag-manage-container">
      ...
      <div class="tags-section">
        <div class="section-header">
          <span class="section-title">{{
            t("dialog.tagManage.globalTags", {
              count: settingsStore.tags.length,
            })
          }}</span>
        </div>
        ...
        <el-scrollbar max-height="300px">
          <div class="tag-list">
            ...
            >
              <div class="tag-info">
                <span class="tag-preview" :style="getTagStyle(tag.color)">
                  {{ tag.name }}
                </span>
                <span class="tag-usage">
                  {{
                    t("dialog.tagManage.usage", {
              ...
              <div class="tag-actions">
                <el-color-picker
                  ...
                  size="small"
                  @change="
                    (color: string | null) => handleUpdateColor(tag.name, color)
                  "
                />
                <el-button
                  :icon="Edit"
      ...
      <div class="batch-section" v-if="selectedAccountIds.length > 0">
        <div class="section-header">
          <span class="section-title">{{
            t("dialog.tagManage.batchOperations", {
        ...
        <div class="batch-content">
          <div class="batch-row">
            <span class="batch-label"
              >{{ t("dialog.tagManage.addTags") }}:</span
            >
            <el-select
          ...
          <div class="batch-row">
            <span class="batch-label"
              >{{ t("dialog.tagManage.removeTags") }}:</span
            >
            <el-select
          ...
          <el-button
            ...
            :disabled="
              batchAddTags.length === 0 && batchRemoveTags.length === 0
            "
          >
...
const availableTagsToAdd = computed(() => {
  ...
  const commonTags = settingsStore.tags
    .map((t) => t.name)
    .filter((tagName) =>
      selectedAccounts.value.every((account) => account.tags.includes(tagName)),
    );
...
async function handleUpdateTag() {
  ...
  // 如果名称改变，检查是否重复
  if (
    name !== editForm.originalName &&
    settingsStore.tags.some((t) => t.name === name)
  ) {
