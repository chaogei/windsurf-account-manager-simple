  ...
  >
    <div v-loading="loading" class="team-container">
      ...
      <el-tabs v-model="activeTab" type="border-card">
        <!-- 团队成员列表 -->
        <el-tab-pane
          :label="$t('dialog.teamManagement.members')"
          name="members"
        >
          <!-- 邀请链接区域 -->
          ...
          >
            <el-table-column
              :label="$t('dialog.teamManagement.nameAndEmail')"
              min-width="220"
            >
            ...
            <el-table-column
              :label="$t('dialog.teamManagement.lastUsed')"
              width="120"
              align="center"
            >
              <template #default="{ row }">
                <span class="time-text">{{
                  formatLastUsed(row.last_update_time)
                }}</span>
              </template>
            </el-table-column>
            <el-table-column
              :label="$t('dialog.teamManagement.usedCredits')"
              width="100"
              align="center"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              :label="$t('dialog.teamManagement.disableAccess')"
              width="90"
              align="center"
            >
              <template #default="{ row }">
                <el-tag
                  :type="row.disable_codeium ? 'danger' : 'success'"
                  size="small"
                >
            ...
            <el-table-column
              :label="$t('dialog.teamManagement.actions')"
              width="180"
              fixed="right"
              align="center"
            >
              <template #default="{ row }">
                ...
                <template v-if="row.role !== 'Admin'">
                  ...
                  <el-button
                    ...
                    size="small"
                    text
                    :loading="row.rejoining"
                    @click="rejoinMember(row)"
                  >
        ...
        <!-- 待处理邀请 -->
        <el-tab-pane
          :label="$t('dialog.teamManagement.pendingInvitations')"
          name="invitations"
        >
          <div class="tab-header">
          ...
          >
            <el-table-column
              prop="name"
              :label="$t('dialog.teamManagement.name')"
              width="150"
            />
            <el-table-column
              prop="email"
              :label="$t('dialog.teamManagement.email')"
              min-width="200"
            />
            <el-table-column
              prop="created_at"
              :label="$t('dialog.teamManagement.invitationTime')"
              width="180"
            >
              <template #default="{ row }">
          ...
          >
            <el-empty
              :description="$t('dialog.teamManagement.noPendingInvitations')"
            />
          </div>
        </el-tab-pane>
        ...
        <!-- 我的邀请（普通用户） -->
        <el-tab-pane
          :label="$t('dialog.teamManagement.myInvitation')"
          name="my-invitation"
        >
          <div class="my-invitation-section">
            <div v-if="myInvitation" class="invitation-card">
              <div class="invitation-info">
                ...
                <p>
                  <strong>{{ $t("dialog.teamManagement.teamName") }}:</strong>
                  {{
                    myInvitation.team_name ||
                    $t("dialog.teamManagement.unknownTeam")
                  }}
                </p>
                <p>
                  <strong>{{ $t("dialog.teamManagement.inviter") }}:</strong>
                  {{
                    myInvitation.admin_name || $t("dialog.teamManagement.admin")
                  }}
                </p>
              </div>
        ...
        <!-- 申请加入团队 -->
        <el-tab-pane
          :label="$t('dialog.teamManagement.joinTeam')"
          name="join-team"
        >
        ...
        <!-- 待审批申请（管理员） -->
        <el-tab-pane
          :label="$t('dialog.teamManagement.pending')"
        ...
        >
          ...
          <el-table :data="pendingMembers" style="width: 100%" max-height="400">
            ...
            <el-table-column
              prop="status"
              :label="$t('dialog.teamManagement.status')"
              width="100"
            >
              <template #default>
                <el-tag type="warning" size="small">{{
                  $t("dialog.teamManagement.pending")
                }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template #default="{ row }">
                <el-button
                  ...
                  size="small"
                  text
                  @click="approveJoinRequest(row, 'approve')"
                >
                ...
                <el-button
                  ...
                  size="small"
                  text
                  @click="approveJoinRequest(row, 'reject')"
                >
          ...
          <div
            v-if="pendingMembers.length === 0 && !loading"
            class="empty-state"
          >
            <el-empty
              :description="$t('dialog.teamManagement.noPendingRequests')"
            />
          </div>
        </el-tab-pane>
      </el-tabs>
    ...
    >
      <el-form :model="inviteForm" label-width="60px">
        <div
          v-for="(user, index) in inviteForm.users"
          :key="index"
        ...
        >
          <div class="invite-user-fields">
            <el-form-item :label="$t('dialog.teamManagement.name')">
              <el-input
                v-model="user.name"
                :placeholder="$t('dialog.teamManagement.memberName')"
              />
            </el-form-item>
            <el-form-item :label="$t('dialog.teamManagement.email')">
              <el-input
                v-model="user.email"
                :placeholder="$t('dialog.teamManagement.memberEmail')"
              />
            </el-form-item>
          </div>
          <el-button
        ...
        <div class="auto-join-section">
          ...
          <span class="auto-join-label">{{
            $t("dialog.teamManagement.autoJoin")
          }}</span>
          <el-tooltip
            :content="$t('dialog.teamManagement.autoJoinTooltip')"
            placement="top"
          >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="inviting" @click="submitInvite">
          {{
            autoJoinEnabled
              ? $t("dialog.teamManagement.inviteAndAutoJoin")
              : $t("dialog.teamManagement.sendInvite")
          }}
        </el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      <div v-if="transferring" class="transfer-progress">
        <el-steps :active="transferStep" finish-status="success" simple>
          <el-step :title="t('dialog.teamManagement.steps.disableAccess')" />
          <el-step :title="t('dialog.teamManagement.steps.inviteUser')" />
          <el-step :title="t('dialog.teamManagement.steps.grantAdmin')" />
          <el-step :title="t('dialog.teamManagement.steps.removeSelf')" />
        </el-steps>
      ...
      <template #footer>
        ...
        >
          {{ $t("dialog.teamManagement.transfer.confirmTransfer") }}
        </el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="selectedMember?.name || $t('dialog.teamManagement.memberDetails')"
      width="520px"
      :close-on-click-modal="false"
      append-to-body
    ...
    >
      <div v-if="selectedMember" class="member-detail-content">
        ...
        <el-form label-width="120px" class="detail-form">
          <el-form-item :label="$t('dialog.teamManagement.apiKey')">
            <el-input
          ...
          <el-form-item :label="$t('dialog.teamManagement.signUpTime')">
            <span class="info-value">{{
              formatSignUpTime(selectedMember.sign_up_time)
            }}</span>
          </el-form-item>

          <el-form-item :label="$t('dialog.teamManagement.role')">
            <el-select
              v-model="memberDetailForm.role"
              style="width: 200px"
              size="default"
            >
              <el-option
                :label="$t('dialog.teamManagement.roles.user')"
                value="User"
              />
              <el-option
                :label="$t('dialog.teamManagement.roles.admin')"
                value="Admin"
              />
            </el-select>
          </el-form-item>

          <el-form-item :label="$t('dialog.teamManagement.disableAccess')">
            <el-switch
              v-model="memberDetailForm.disableAccess"
              :active-text="$t('dialog.teamManagement.disabled')"
              inactive-text=""
              style="--el-switch-on-color: #f56c6c"
            />
            <div class="form-tip">
              {{ $t("dialog.teamManagement.disableTip") }}
            </div>
          </el-form-item>
        </el-form>
      </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          >
            {{ $t("dialog.teamManagement.removeMember") }}
          </el-button>
          <el-button
            type="primary"
            @click="saveMemberDetail"
            :loading="memberDetailLoading"
          >
            {{ $t("dialog.teamManagement.saveChanges") }}
          </el-button>
        </div>
      </template>
    </el-dialog>
...
const otherMembers = computed(() => {
  return members.value.filter(
    (m) => m.email?.toLowerCase() !== currentAccountEmail.value?.toLowerCase(),
  );
});
...
async function loadTeamMembers() {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (Array.isArray(users) && users.length > 0) {
        for (const user of users) {
          ...
          if (Array.isArray(cascadeDetails)) {
            // 如果是数组
            const usageInfo = cascadeDetails.find(
              (c: any) => c.string_1 === firebaseId,
            );
            if (usageInfo) {
      ...
      ElMessage.error(
        result.error || t("dialog.teamManagement.operationFailed"),
      );
    }
  } catch (error: any) {
...
async function removeMember(member: TeamMember) {
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(t("dialog.teamManagement.memberRemoved"));
      loadTeamMembers();
    } else {
      ElMessage.error(
        result.error || t("dialog.teamManagement.removeMemberFailed"),
      );
    }
  } catch (error: any) {
    ElMessage.error(error.toString());
...
function copyApiKey() {
  ...
  navigator.clipboard
    .writeText(selectedMember.value.api_key)
...
function formatLastUsed(timestamp?: number): string {
  if (!timestamp) return t("common.neverUsed");
...
async function saveMemberDetail() {
  ...
  try {
    // 标准化角色值进行比较
    let originalRole =
      typeof selectedMember.value.role === "string"
    ...
    if (originalRole !== newRole) {
      ...
      if (newRole !== "User") {
        const newRoleApi =
          newRole === "Admin" ? "root.admin" : `root.${newRole.toLowerCase()}`;
...
async function handleRemoveMember() {
  ...
  try {
    await ElMessageBox.confirm(
      t("dialog.teamManagement.confirmRemoveMemberMsg"),
      t("common.confirm"),
      {
...
async function rejoinMember(member: TeamMember) {
  ...
  try {
    ...
    if (!removeResult.success) {
      ElMessage.error(
        removeResult.error || t("dialog.teamManagement.removeMemberFailed"),
      );
      return;
    }
    ...
    if (!inviteResult.success) {
      ElMessage.error(
        inviteResult.error || t("dialog.teamManagement.reinviteFailed"),
      );
      loadTeamMembers();
      return;
    }
    ...
    if (joined) {
      ElMessage.success(
        t("dialog.teamManagement.resetCreditsSuccess", { name: member.name }),
      );
    } else {
      ElMessage.success(
        t("dialog.teamManagement.reinvitedWait", { name: member.name }),
      );
    }
...
async function batchResetMemberCredits() {
  ...
  if (membersToReset.length === 0) {
    ElMessage.warning(t("dialog.teamManagement.noMembersToReset"));
    return;
  }

  try {
    await ElMessageBox.confirm(
      t("dialog.teamManagement.confirmBatchResetMsg", {
        count: membersToReset.length,
      }),
      t("dialog.teamManagement.batchResetTitle"),
      {
        confirmButtonText: t("common.yes"),
        cancelButtonText: t("common.no"),
        type: "warning",
  ...
  try {
    for (const member of membersToReset) {
      try {
        ...
        if (!removeResult.success) {
          console.error(
            `Failed to remove member ${member.name}:`,
            removeResult.error,
          );
          failCount++;
          continue;
        ...
        if (!inviteResult.success) {
          console.error(
            `Failed to invite member ${member.name}:`,
            inviteResult.error,
          );
          failCount++;
          continue;
    ...
    if (failCount === 0) {
      ElMessage.success(
        t("dialog.teamManagement.batchResetSuccess", { count: successCount }),
      );
    } else {
      ElMessage.warning(
        t("dialog.teamManagement.batchResetPartial", {
...
async function loadPendingInvitations() {
  ...
  try {
    ...
    if (result.success) {
      ...
      ElMessage.error(
        result.error || t("dialog.teamManagement.fetchPendingFailed"),
      );
    }
  } catch (error: any) {
    ElMessage.error(error.toString());
...
async function revokeInvitation(invitation: PendingInvitation) {
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(t("dialog.teamManagement.revokeInvitationSuccess"));
      loadPendingInvitations();
    } else {
      ElMessage.error(
        result.error || t("dialog.teamManagement.revokeInvitationFailed"),
      );
    }
  } catch (error: any) {
    ElMessage.error(error.toString());
...
async function acceptInvitation() {
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(t("dialog.teamManagement.joinTeamSuccess"));
      myInvitation.value = null;
    } else {
      ElMessage.error(
        result.error || t("dialog.teamManagement.joinTeamFailed"),
      );
    }
  } catch (error: any) {
    ElMessage.error(error.toString());
...
async function rejectInvitation() {
  ...
  try {
    await ElMessageBox.confirm(
      t("dialog.teamManagement.confirmRejectInvitationMsg"),
      t("common.confirm"),
      {
        type: "warning",
      },
    ...
    if (result.success) {
      ElMessage.success(t("dialog.teamManagement.rejectInvitationSuccess"));
      myInvitation.value = null;
    } else {
      ElMessage.error(
        result.error || t("dialog.teamManagement.rejectInvitationFailed"),
      );
    }
  } catch (error: any) {
    if (error !== "cancel") {
...
async function submitInvite() {
  ...
  if (validUsers.length === 0) {
    ElMessage.warning(t("dialog.teamManagement.invalidMemberInfo"));
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      if (autoJoinEnabled.value) {
        const autoJoinResults = await autoAcceptInvitations(
        ...
        if (joinedCount > 0) {
          ElMessage.success(
            t("dialog.teamManagement.inviteSuccessWithAutoJoin", {
              invitedCount,
              joinedCount,
            }),
          );
          ...
          ElMessage.success(
            t("dialog.teamManagement.inviteSuccess", { count: invitedCount }),
          );
        }
      } else {
        ElMessage.success(
      ...
      ElMessage.error(result.error || t("dialog.teamManagement.inviteFailed"));
    }
  } catch (error: any) {
    ElMessage.error(error.toString());
  } finally {
...
async function autoAcceptInvitations(
  ...
  try {
    ...
    for (const email of emails) {
      ...
      if (matchedAccount) {
        console.log(
          `[AutoJoin] Found account for ${email}, attempting to accept invitation...`,
        );
        ...
        try {
          ...
          console.error(
            `[AutoJoin] Failed to accept invitation for ${email}:`,
            e,
          );
...
async function submitJoinRequest() {
  if (!joinForm.value.inviteId.trim()) {
    ElMessage.warning(t("dialog.teamManagement.inviteIdPlaceholder"));
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(
        result.message || t("dialog.teamManagement.joinRequestSubmitted"),
      );
      joinForm.value.inviteId = "";
    } else {
      ElMessage.error(
        result.error || t("dialog.teamManagement.joinRequestFailed"),
      );
    }
  } catch (error: any) {
    ElMessage.error(error.toString());
  ...
  try {
    ...
    if (result.success) {
      ElMessage.success(
        result.message ||
          (action === "approve"
            ? t("dialog.teamManagement.approved")
            : t("dialog.teamManagement.rejected")),
      );
      loadTeamMembers();
    } else {
      ElMessage.error(
...
async function executeTransfer() {
  if (!transferForm.value.email.trim()) {
    ElMessage.warning(t("dialog.teamManagement.transfer.placeholder"));
    return;
  }

  try {
    await ElMessageBox.confirm(
      t("dialog.teamManagement.transfer.confirmMsg", {
        email: transferForm.value.email,
      }),
      t("dialog.teamManagement.transfer.confirmTitle"),
      {
        confirmButtonText: t("dialog.teamManagement.transfer.confirmBtn"),
  ...
  try {
    ...
    const targetName =
      transferForm.value.name.trim() || targetEmail.split("@")[0];

    // Step 1: 禁用自己的访问权限
    transferStatus.value = t(
      "dialog.teamManagement.transfer.status.disablingSelf",
    );
    transferStep.value = 0;
    ...
    const currentMember = members.value.find(
      (m) =>
        m.email?.toLowerCase() === currentAccountEmail.value?.toLowerCase(),
    );
    ...
    // Step 2: 邀请用户加入团队
    transferStatus.value = t("dialog.teamManagement.transfer.status.inviting");
    ...
    if (!existingMember) {
      ...
      if (!inviteResult.success) {
        throw new Error(
          inviteResult.error || t("dialog.teamManagement.inviteFailed"),
        );
      }
      ...
      if (!joined) {
        // 用户未能自动加入，需要等待手动接受
        ElMessage.warning(
          t("dialog.teamManagement.transfer.manualAcceptNeeded"),
        );
      ...
      const newMember = members.value.find(
        (m) => m.email?.toLowerCase() === targetEmail.toLowerCase(),
      );
      if (!newMember) {
        throw new Error(t("dialog.teamManagement.transfer.memberNotFound"));
      }
    ...
    // Step 3: 赋予用户管理员权限
    transferStatus.value = t(
      "dialog.teamManagement.transfer.status.grantingAdmin",
    );
    ...
    // Step 4: 把自己移除团队
    transferStatus.value = t(
      "dialog.teamManagement.transfer.status.removingSelf",
    );
    ...
    transferStatus.value = t("dialog.teamManagement.transfer.status.completed");

    ElMessage.success(
      t("dialog.teamManagement.transfer.success", { email: targetEmail }),
    );
    ...
    ElMessage.error(
      t("dialog.teamManagement.transfer.failed", {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.data?.user?.email) {
        ...
        console.log(
          "[TeamManagement] Current account email:",
          currentAccountEmail.value,
        );
      }
    }
  } catch (error: any) {
...
async function copyInviteId() {
  ...
  try {
    ...
    ElMessage.success(t("dialog.teamManagement.copyInviteIdSuccess"));
  } catch (error) {
    // 备用方法
...
async function copyInviteUrl() {
  ...
  try {
    ...
    ElMessage.success(t("dialog.teamManagement.copyInviteUrlSuccess"));
  } catch (error) {
    const textarea = document.createElement("textarea");
